    ...
    export class AccountAdapter {
        static async login(): Promise<any> {
            return new Promise((resolve, reject) => {
                platform.login().then(wxRes => {
                    ...
                    request.open(`${ap.Constants.Endpoints.service}users/login?code=${wxRes.code}`, egret.HttpMethod.POST);
                    ...
                    request.addEventListener(egret.Event.COMPLETE, (event: egret.Event) => {
                        ...
                        if (res.error) {
                            ...
                            ap.CommonData.logon = { ...ap.CommonData.logon, ...res.data };  //this is the unique Id.
        ...
        static async checkForUpdate() {
            return new Promise(async (resolve, reject) => {
                ...
                var request = new egret.HttpRequest();
                request.responseType = egret.HttpResponseType.TEXT;
                request.open(`${ap.Constants.Endpoints.service}version/check?version=${version}`, egret.HttpMethod.GET);
                request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                request.send();
                request.addEventListener(egret.Event.COMPLETE, (event: egret.Event) => {
                    let req = <egret.HttpRequest>(event.currentTarget);
